...
struct EnemyN//ノーマルレベル敵クラス
{
	double EInterval = 0.6;//敵の出現間隔
	double etime = 0;
	...
	void EMovement(int& score)
	{
		...
		if (score < 25)
		{
			...
			{
				enemy.y += Scene::DeltaTime() * 240;//毎秒200px
			}
		}
		else if (score >= 25&& score < 50)
		{
			...
			for (auto& enemy : enemies_down)
			{
				enemy.y += Scene::DeltaTime() * 420;//毎秒320px
			}
		...
		else if (score >= 50)
		{
			...
			for (auto& enemy : enemies_down)
			{
				enemy.y += Scene::DeltaTime() * 400;//毎秒300px
			}
			for (auto& enemy : enemies_right)
			{
				enemy.x += Scene::DeltaTime() * 500;//毎秒400px
			}
